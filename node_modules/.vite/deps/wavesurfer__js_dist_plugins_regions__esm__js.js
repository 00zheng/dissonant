...
function h(t2, e2 = {}) {
  ...
  const c = (e3) => {
    ...
    const p = Date.now(), v = t2.getBoundingClientRect(), { left: g, top: m } = v, f = (t3) => {
      if (t3.pointerId !== s2) return;
      if (t3.defaultPrevented || h2.size > 1) return;
      if (a2 && Date.now() - p < r2) return;
      const e4 = t3.clientX, n3 = t3.clientY, d3 = e4 - l2, v2 = n3 - c2;
      (u || Math.abs(d3) > i2 || Math.abs(v2) > i2) && (t3.preventDefault(), t3.stopPropagation(), u || (o2.set({ type: "start", x: l2 - g, y: c2 - m }), u = true), o2.set({ type: "move", x: e4 - g, y: n3 - m, deltaX: d3, deltaY: v2 }), l2 = e4, c2 = n3);
    }, b = (t3) => {
