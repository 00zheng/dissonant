...
import { lanczosKernel, lanczosStrategy, registerLanczosStrategy, } from './index.js';
describe('lanczos interpolation strategy', () => {
    describe('lanczosKernel.createState', () => {
        it('returns default state values', () => {
            const state = lanczosKernel.createState?.();
    ...
    describe('lanczosStrategy', () => {
        it('normalizes zeroCrossings and normalize and applies params to state', () => {
            const normalizedLow = lanczosStrategy.normalizeParams?.({ zeroCrossings: 1, normalize: 0 }, lanczosStrategy.defaultParams ?? {});
            const normalizedHigh = lanczosStrategy.normalizeParams?.({ zeroCrossings: 99, normalize: 1 }, lanczosStrategy.defaultParams ?? {});
            const normalizedUndefined = lanczosStrategy.normalizeParams?.(undefined, {});
            ...
            expect(state.params.zeroCrossings).toBe(4);
            expect(() => lanczosStrategy.applyParams?.(null, { zeroCrossings: 6 })).not.toThrow();
        });
